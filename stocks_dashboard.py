...
for symbol in stock_symbols:
    try:
        ...
        if not real_time_data.empty and 'Open' in real_time_data.columns and 'Close' in real_time_data.columns:
            real_time_data = process_data(real_time_data)
            last_price = real_time_data['Close'].iloc[-1]
            change = last_price - real_time_data['Open'].iloc[0]
            pct_change = (change / real_time_data['Open'].iloc[0]) * 100
            
            # Ensure last_price and change are scalars
            st.sidebar.metric(f"{symbol}", f"{last_price:.2f} USD", f"{change:.2f} ({pct_change:.2f}%)")
        else:
            st.sidebar.metric(f"{symbol}", "Data not available", "")
    except Exception as e:
        st.sidebar.metric(f"{symbol}", "Error", str(e))
